...
TARGET_SPEC = {"width": 480, "height": 640, "dpi": 300}
BLUE_RGB = (98, 139, 206)
...
def make_options() -> dict:
    return {"background": "blue", "renderMode": "solid", "width": 480, "height": 640, "dpi": 300}


def test_resize_candidate_passes_with_warning() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert_true(report["passed"] is True, "resizable same-background candidate should pass")
        assert_true(report["usableImagePath"].endswith(".png"), "usable resized derivative should be emitted")
        assert_true(report["checks"]["dimensions"]["derivative"], "dimension derivative metadata should be present")
...
def test_background_deviation_fails_to_fallback() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write_solid(candidate, (255, 0, 0), (480, 640))
        report = evaluate_ai_pro_quality(
            ai_image_path=candidate,
            output_dir=root,
            task_id="task_bad_bg",
...
def main() -> None:
    test_resize_candidate_passes_with_warning()
