...
const backgrounds: Array<{ id: BackgroundColor; label: string; swatch: string; note: string }> = [
  { id: 'white', label: '白底', swatch: '#f9f9f6', note: '通用证件' },
  { id: 'blue', label: '蓝底', swatch: '#8fb7d6', note: '考试 / 档案' },
  { id: 'red', label: '红底', swatch: '#b84a42', note: '政务材料' },
  { id: 'gray', label: '灰底', swatch: '#c9c6bf', note: '签证预览' },
];
    ...
    <div className="grid gap-4 lg:grid-cols-[1.1fr_.9fr]">
      <section className="precision-card rounded-[24px] p-5">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="font-mono text-[11px] uppercase tracking-[0.28em] text-slate">Specification ruler</p>
            <h3 className="mt-2 text-xl font-semibold tracking-[-0.03em]">选择证件照规格</h3>
          </div>
      ...
      <section className="precision-card rounded-[24px] p-5">
        <p className="font-mono text-[11px] uppercase tracking-[0.28em] text-slate">Background cassette</p>
        <h3 className="mt-2 text-xl font-semibold tracking-[-0.03em]">选择底色</h3>
        <div className="mt-4 grid grid-cols-2 gap-3">
          {backgrounds.map((background) => {
            const active = background.id === selectedBackground;
            return (
              <button
                key={background.id}
                type="button"
              ...
              >
                <span className="block h-8 rounded-xl border border-ink/10" style={{ background: background.swatch }} />
                <span className="mt-3 block font-semibold">{background.label}</span>
                <span className="text-xs text-slate">{background.note}</span>
              </button>
            );
          })}
        </div>
        ...
        <div className="mt-4 rounded-[18px] border border-ink/10 bg-ink p-4 text-porcelain">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[0.26em] text-[#b7c9cc]">Optional branch</p>
              <h4 className="mt-2 font-semibold">AI Enhance preview</h4>
              <p className="mt-2 text-xs leading-5 text-[#d8d1c4]">Coming soon: separated from official result and server-side only.</p>
            </div>
        ...
        >
          {isWorking ? 'Advancing task…' : 'Create mock task'}
        </button>
      </section>
    </div>
  );
